...
![AI MCP 系统架构图](aws_cloud_mcp_architecture_optimized.png)

#### 架构概述

系统采用分阶段设计，通过5个关键阶段实现AI智能客服：

1. **⭕ 1️⃣ 用户请求接收** - 通过API Gateway接收用户查询
2. **⭕ 2️⃣ AI智能分析** - 使用AWS Bedrock进行意图理解和工具选择
3. **⭕ 3️⃣ 工具发现与调用** - 通过MCP协议调用相应的工具服务
4. **⭕ 4️⃣ 数据获取与处理** - 从外部API获取业务数据
5. **⭕ 5️⃣ 智能回复生成** - 生成自然语言回复返回给用户

#### 技术栈

- **☁️ AWS Cloud Platform**: 完全基于AWS云原生架构
- **⚡ Serverless Computing**: Lambda函数按需扩展，成本效益高
- **🧠 AI/ML服务**: Amazon Bedrock提供企业级AI能力
- **🔧 MCP协议**: 标准化的工具调用和集成框架
- **🛡️ 安全与监控**: IAM权限控制和CloudWatch全方位监控
