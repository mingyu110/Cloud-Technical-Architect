...
def load_dataset(directory, file_extension: str):
    allowed_extensions = ["train", "test", "val"]
    if file_extension not in allowed_extensions:
        raise ValueError("Invalid extension. Expected one of: %s" % allowed_extensions)

    x = np.load(os.path.join(directory, f"x_{file_extension}.npy"))
    y = np.load(os.path.join(directory, f"y_{file_extension}.npy"))
